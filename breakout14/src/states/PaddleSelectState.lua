...
function PaddleSelectState:update(dt)
    ...
    if love.keyboard.wasPressed('return') or love.keyboard.wasPressed('enter') then
        ...
        --create level
        local levelBricks = LevelMaker.createMap(32)

        -- check if level has locked brick
        local hasSpecialBrick = false
        for k, brick in pairs(levelBricks) do
        ...
        gStateMachine:change('serve', {
            ...
            level = 1,
            recoverPoints = 5000,
            --parametr for special brick
            keyMode = hasSpecialBrick and 1 or 0
        })
    end
...
function PaddleSelectState:render()
    ...
    if self.currentPaddle == 1 then
        ...
        love.graphics.setColor(love.math.colorFromBytes(40, 40, 40, 128))
    end
