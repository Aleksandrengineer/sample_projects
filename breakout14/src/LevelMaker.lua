...
function LevelMaker.createMap(level)
    ...
    --if level is greater than 15, add a locked brick
    local hasUnlockableBrick = level > 15
    ...
    if #bricks == 0 then
        ...
        --turn a random brick into a unlockable one
        if (#bricks > 1 and hasUnlockableBrick) then
            local brickIndex = math.random(#bricks)
            bricks[brickIndex]:lock()
        end
